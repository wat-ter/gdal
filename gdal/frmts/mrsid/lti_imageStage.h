...
public:
    ...
    *
    * @param pixelProps the band selection
    * @param fullScene  the full scene to be read
    * @return status code indicating success or failure
    */
   virtual LT_STATUS readBegin(const LTIPixel &pixelProps,
    ...
    * @return  status code indicating success or failure
    */
   virtual LT_STATUS readStrip(LTISceneBuffer &buffer,
   ...
   /**
    * override the colorspace of all the pixel properties (PixelProps, NoData, Background, Min/MaxDynamicRange)
    */
   virtual LT_STATUS overrideColorSpace(LTIColorSpace colorspace) = 0;
...
protected:
    ...
    *
    * This function sets the buffer to the background pixel if there is one, 
    * otherwise, the nodata pixel if there is one
    * otherwise the background will be filled with black.
    *
    * @param   scene   the region (and scale) of the image to be read
    * @param   buffer  the buffer to read the pixels into
    * @return  status code indicating success or failure
    ...
    * @return status code indicating success or failure
    */
   virtual LT_STATUS decodeBegin(const LTIPixel &pixelProps,
                                 const LTIScene &fullScene) = 0;
...
private:
   /**
    * validate the a read request for this image stage
    */
   LT_STATUS validateReadRequest(const LTIPixel &pixelProps,
                                 const LTIScene &scene) const;
   LT_STATUS validateReadRequest(const LTISceneBuffer &buffer,
                                 const LTIScene &scene) const;
